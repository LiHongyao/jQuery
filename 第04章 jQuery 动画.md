...
- **easing**：用定切换效果，默认是"`swing`"，可用参数"`linear`"
...
> 3. 如果让 show 与 hide 成为一个动画，那么默认执行动画会改变元素的高度，高度，透明度。
...
对于隐藏的元素，在将其显示出来的过程中，可以对其进行一些变化的动画效果。之前学过了show方法，show方法在显示的过程中也可以有动画，但是 `.show()` 方法将会匹配元素的宽度，高度，以及不透明度，同时进行动画操作。这里将要学习一个新的显示方法 `.slideDown()` 方法，用滑动动画显示一个匹配元素。
...
- `prop`：一个或多个css属性的键值对所构成的Object对象，对 **数字属性值** 有效，驼峰命名，可使用快捷方式如‘show/toggle’。
